...
private:
  ...
  rclcpp::Publisher<rm_ros_interfaces::msg::Setforceposition>::SharedPtr
    set_force_postion_publisher_;                                                                       //声明发布器
...
/******************************接收到订阅的机械臂执行状态消息后，会进入消息回调函数**************************/
void ForcePositionControlDemoSub::SetForcePostionDemo_Callback(
...
/******************************接收到订阅的机械臂执行状态消息后，会进入消息回调函数**************************/
void ForcePositionControlDemoSub::StopForcePostionDemo_Callback(
  const std_msgs::msg::Bool::SharedPtr msg)
{
  // 将接收到的消息打印出来，显示是否执行成功
...
/***********************************构造函数，初始化发布器订阅器****************************************/
ForcePositionControlDemoPub::ForcePositionControlDemoPub()
: rclcpp::Node("Force_Position_Control_pub_node")
{
  movej_p_publisher_ = this->create_publisher<rm_ros_interfaces::msg::Movejp>(
    "rm_driver/movej_p_cmd", rclcpp::ParametersQoS());
  movel_publisher_ = this->create_publisher<rm_ros_interfaces::msg::Movel>(
    "rm_driver/movel_cmd",
  ...
  stop_force_postion_publisher_ = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/stop_force_postion_cmd", rclcpp::ParametersQoS());
  loop_pub_Timer = this->create_wall_timer(
...
/***********************************构造函数，初始化发布器订阅器****************************************/
ForcePositionControlDemoSub::ForcePositionControlDemoSub()
: rclcpp::Node("Force_Position_Control_sub_node")
{
  ...
  movel_subscription_ = this->create_subscription<std_msgs::msg::Bool>(
    ...
    rclcpp::ParametersQoS(), std::bind(&ForcePositionControlDemoSub::MoveLDemo_Callback, this, _1));
