...
def generate_launch_description():
    ...
    robot_description_config = xacro.process_file(
        os.path.join(
            ...
            "config",
            "rm_eco62_description.urdf.xacro",
        )
    )
    ...
    robot_description_semantic_config = load_file(
        package_name, "config/rm_eco62_description.srdf"
    )
    ...
    kinematics_yaml = load_yaml(
        package_name, "config/kinematics.yaml"
    )
    ...
    ompl_planning_yaml = load_yaml(
        package_name, "config/ompl_planning.yaml"
    )
    ...
    rviz_base = os.path.join(get_package_share_directory(package_name), "config")
    ...
    load_controllers = []
    for controller in ["rm_group_controller"]:
        load_controllers += [
            ExecuteProcess(
