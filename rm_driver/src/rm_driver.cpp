...
{
public:
  explicit ShutdownRequested(const std::string & message)
...
std::string normalize_ipv4_address(const std::string & address)
{
  ...
  if (inet_ntop(AF_INET, &binary_address, normalized, sizeof(normalized)) == nullptr) {
    throw std::runtime_error("Failed to normalize arm_ip " + address + ": " + strerror(errno));
  }
  return normalized;
}
...
void Acquire_Arm_Ownership(const std::string & ip, int port)
{
  ...
  if (flock(lock_fd, LOCK_EX | LOCK_NB) != 0) {
    ...
    if (lock_error == EWOULDBLOCK || lock_error == EAGAIN) {
      throw std::runtime_error(
              "Robot endpoint " + endpoint + " is already owned by another rm_driver process");
    }
    throw std::runtime_error(
...
int Arm_Socket_Start_Connect(void)
{
  ...
  if (Arm_connect != 0) {
    if (errno != EINPROGRESS) {  // connect返回错误。
      ...
      if (res <= 0) {
        std::cout << "********************Connect faile check your connect!**************" <<
          std::endl;
        close(Arm_Socket);
        return 3;
      }
...
int Arm_Start(void)
{
  ...
  if (new_handle->id < 0) {
    const int create_result = new_handle->id;
...
void RmArm::Arm_Set_Realtime_Push_Callback(
  const rm_ros_interfaces::msg::Setrealtimepush::SharedPtr msg)
{
  rm_realtime_push_config_t config;
  int32_t res;
  ...
  config.enable = true;
  std::snprintf(config.ip, sizeof(config.ip), "%s", msg->ip.c_str());
...
void RmArm::Set_UDP_Configuration(
  int udp_cycle, int udp_port, int udp_force_coordinate,
  std::string udp_ip, bool hand, bool rm_plus_base,
  bool rm_plus_state)
{
  int32_t res;
  rm_realtime_push_config_t config;
  ...
  config.enable = true;
  std::snprintf(config.ip, sizeof(config.ip), "%s", udp_ip.c_str());
  rm_udp_custom_config_t config_enable;
  ...
  if (res == 0) {
    RCLCPP_INFO(
      this->get_logger(), "UDP_Configuration is cycle:%dms,port:%d,force_coordinate:%d,ip:%s,hand:%d,rm_plus_base:%d,rm_plus_state:%d,speed:%d\n", udp_cycle, udp_port, udp_force_coordinate,
      udp_ip.c_str(), udp_hand_g, rm_plus_base_g, rm_plus_state_g,
      config.custom_config.joint_speed);
  } else {
    RCLCPP_INFO(this->get_logger(), "The set_realtime_push error code is %d\n", res);
  }
}
...
void RmArm::Get_Trajectory_File_List_Callback(
  const rm_ros_interfaces::msg::Gettrajectorylist::SharedPtr msg)  // 查询轨迹列表
{
  int32_t res;
  ...
  res = Rm_Api.rm_get_trajectory_file_list(
    robot_handle, page_num, page_size, vague_search,
...
void RmArm::Arm_Get_Flowchart_Program_Run_State_Callback(
  const std_msgs::msg::Empty::SharedPtr msg)  // 不支持三代控制器
{
  int32_t res;
...
// ------------------------------------------Modbus相关------------------------------------------
void RmArm::Add_Modbus_Tcp_Master_Callback(
  const rm_ros_interfaces::msg::Modbustcpmasterinfo::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  std::snprintf(
    master.master_name, sizeof(master.master_name), "%s", msg->master_name.c_str());
  std::snprintf(master.ip, sizeof(master.ip), "%s", msg->ip.c_str());
...
void RmArm::Update_Modbus_Tcp_Master_Callback(
  const rm_ros_interfaces::msg::Modbustcpmasterupdata::SharedPtr msg)
{
  int32_t res;
  rm_modbus_tcp_master_info_t master;
  // copy = msg;
  std_msgs::msg::Bool Update_Modbus_Tcp_Master_result;
  char old_master_name[14] {};
  std::snprintf(
    old_master_name, sizeof(old_master_name), "%s", msg->master_name.c_str());
  std::snprintf(
    master.master_name, sizeof(master.master_name), "%s", msg->new_master_name.c_str());
  std::snprintf(master.ip, sizeof(master.ip), "%s", msg->ip.c_str());
  master.port = msg->port;
...
void RmArm::Delete_Modbus_Tcp_Master_Callback(
  const rm_ros_interfaces::msg::Mastername::SharedPtr msg)
{
  int32_t res;
  char master_name[20];
  std_msgs::msg::Bool Delete_Modbus_Tcp_Master_result;
  std::snprintf(master_name, sizeof(master_name), "%s", msg->master_name.c_str());
...
void RmArm::Get_Modbus_Tcp_Master_List_Callback(
  const rm_ros_interfaces::msg::Getmodbustcpmasterlist::SharedPtr msg)
{
  int32_t res;
  ...
  page_num = msg->page_num;
  page_size = msg->page_size;
  std::snprintf(vague_search, sizeof(vague_search), "%s", msg->vague_search.c_str());

  res = Rm_Api.rm_get_modbus_tcp_master_list(
    robot_handle, page_num, page_size, vague_search,
    &master_list);
...
void RmArm::Set_Controller_RS485_Mode_Callback(
  const rm_ros_interfaces::msg::RS485params::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
...
void RmArm::Set_Controller_Tcp_Mode_Callback(
  const rm_ros_interfaces::msg::Modbustcpmasterinfo::SharedPtr msg)
{
  int32_t res;
...
{
  int32_t res;
  int tool_rs485_mode = 0, baudrate = 0, timeout = 0;
  ...
  if (res == 0) {
    ...
    this->Get_Controller_RS485_Mode_v4_Result->publish(controller_rs485_mode_get_result);
    RCLCPP_INFO(this->get_logger(), "The get_controller_rs485_mode error code is %d\n", res);
  }
}
...
void RmArm::Get_Tool_RS485_Mode_v4_Callback(const std_msgs::msg::Empty::SharedPtr msg)
{
  ...
  if (res == 0) {
    ...
    this->Get_Tool_RS485_Mode_v4_Result->publish(tool_rs485_mode_get_result);
    RCLCPP_INFO(this->get_logger(), "The get_tool_rs485_mode error code is %d\n", res);
  }
}

void RmArm::Read_Modbus_RTU_Coils_Callback(
  const rm_ros_interfaces::msg::Modbusrtureadparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  if (res == 0) {
    if (controller_type == 4) {
      ...
      if (static_cast<int>((params_coils.num + 7) / 8) <= 1) {
...
void RmArm::Write_Modbus_RTU_Coils_Callback(
  const rm_ros_interfaces::msg::Modbusrtuwriteparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  param.num = msg->num;
  if (controller_type == 4) {
    if ((param.num == static_cast<int>(msg->data.size())) && (param.num <= 100)) {
      for (int i = 0; i < param.num; i++) {
      ...
      RCLCPP_WARN(
        this->get_logger(), "The write_modbus_rtu_coils num is diff with data size use data size %d\n",
        param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
    ...
    if (param.num <= 1) {
      ...
      if (static_cast<int>(msg->data.size()) == (param.num) / 8) {
        if (param.num <= 160) {
          params_coils.num = param.num;
        } else if ((param.num > 160)) {
          params_coils.num = 160;
        }
      } else if (static_cast<int>(msg->data.size() * 8) > param.num) {
        if (param.num < 160) {
          params_coils.num = param.num;
        }
      } else {
      ...
      int coil_data[25];
      for (int i = 0; i < static_cast<int>(msg->data.size()); i++) {
...
void RmArm::Read_Modbus_RTU_Input_Status_Callback(
  const rm_ros_interfaces::msg::Modbusrtureadparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
...
void RmArm::Read_Modbus_RTU_Holding_Registers_Callback(
  const rm_ros_interfaces::msg::Modbusrtureadparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  if (controller_type == 4) {
    ...
    if (param.num <= 1) {
      ...
      if (param.num > 12) {
        param.num = 12;
        RCLCPP_WARN(
          this->get_logger(), "The read_modbus_rtu_holding_registers num is over 12 use 12\n");
      }
      params_coils.num = param.num;
...
void RmArm::Write_Modbus_RTU_Registers_Callback(
  const rm_ros_interfaces::msg::Modbusrtuwriteparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  if (controller_type == 4) {
    if ((param.num == static_cast<int>(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(
        this->get_logger(), "The write_modbus_rtu_registers num is diff with data size use data size %d\n",
        param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
    ...
    if (param.num <= 1) {
      ...
      if (static_cast<int>(msg->data.size()) == param.num * 2) {
        if (param.num <= 10) {
          params_coils.num = param.num;
...
void RmArm::Read_Modbus_RTU_Input_Registers_Callback(
  const rm_ros_interfaces::msg::Modbusrtureadparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  if (controller_type == 4) {
    ...
    if (param.num <= 1) {
      ...
      if (param.num > 12) {
        param.num = 12;
        RCLCPP_WARN(
          this->get_logger(), "The read_modbus_rtu_input_registers num is over 12 use 12\n");
      }
      params_coils.num = param.num;
...
void RmArm::Read_Modbus_TCP_Coils_Callback(
  const rm_ros_interfaces::msg::Modbustcpreadparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  param.address = msg->address;
  std::snprintf(
    param.master_name, sizeof(param.master_name), "%s", msg->master_name.c_str());
  std::snprintf(param.ip, sizeof(param.ip), "%s", msg->ip.c_str());
  param.port = msg->port;
  param.num = msg->num;
...
void RmArm::Write_Modbus_TCP_Coils_Callback(
  const rm_ros_interfaces::msg::Modbustcpwriteparams::SharedPtr msg)
{
  int32_t res;
  ...
  if (controller_type == 4) {
    if ((param.num == static_cast<int>(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(
        this->get_logger(), "The write_modbus_tcp_coils num is diff with data size use data size %d\n",
        param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
...
void RmArm::Read_Modbus_TCP_Input_Status_Callback(
  const rm_ros_interfaces::msg::Modbustcpreadparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
...
void RmArm::Read_Modbus_TCP_Holding_Registers_Callback(
  const rm_ros_interfaces::msg::Modbustcpreadparams::SharedPtr msg)
{
  int32_t res = -1;
  // copy = msg;
  ...
  if (controller_type == 4) {
    ...
    RCLCPP_ERROR(
      this->get_logger(),
      "Cannot read Modbus TCP holding registers: unsupported controller_type %d",
      controller_type);
  }
  if (res == 0) {
    if (controller_type == 4) {
    ...
    this->Read_Modbus_TCP_Holding_Registers_Result->publish(tcp_read_holding_registers_data);
    RCLCPP_INFO(
      this->get_logger(), "The read_modbus_tcp_holding_registers error code is %d\n",
      res);
  }
}

void RmArm::Write_Modbus_TCP_Registers_Callback(
  const rm_ros_interfaces::msg::Modbustcpwriteparams::SharedPtr msg)
{
  int32_t res;
  // copy = msg;
  ...
  if (controller_type == 4) {
    if ((param.num == static_cast<int>(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(
        this->get_logger(), "The write_modbus_tcp_registers num is diff with data size use data size %d\n",
        param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
    ...
    if (param.num <= 1) {
      ...
      if (static_cast<int>(msg->data.size() / 2) == param.num) {
        if (param.num <= 10) {
          params_coils.num = param.num;
        } else if ((param.num > 10)) {
          params_coils.num = 10;
        }
      } else if (static_cast<int>(msg->data.size()) != param.num) {
...
void RmArm::Read_Modbus_TCP_Input_Registers_Callback(
  const rm_ros_interfaces::msg::Modbustcpreadparams::SharedPtr msg)
{
  int32_t res = -1;
  // copy = msg;
  ...
  if (controller_type == 4) {
    if (param.num > 100) {
      param.num = 100;
      RCLCPP_WARN(
        this->get_logger(), "The read_modbus_tcp_input_registers num is over 100 use 100\n");
    }
    ...
    RCLCPP_ERROR(
      this->get_logger(),
      "Cannot read Modbus TCP input registers: unsupported controller_type %d",
      controller_type);
  }
  if (res == 0) {
    if (controller_type == 4) {
...
void RmArm::Send_Project_Callback(const rm_ros_interfaces::msg::Sendproject::SharedPtr msg)  // 文件下发
{
  ...
  int errline;
  std::snprintf(
    project.project_path, sizeof(project.project_path), "%s", msg->project_path.c_str());
  ...
  if (res == 0) {
    ...
    if (res == 1) {
      if (errline == 0) {
        RCLCPP_INFO(
          this->get_logger(), "The rm_send_project length of the verification data is incorrect");
      } else if (errline == -1) {
        RCLCPP_INFO(this->get_logger(), "rm_send_project no error");
      } else {
        RCLCPP_INFO(
          this->get_logger(), "The rm_send_project number of problematic engineering lines：%d",
          errline);
      }
    }
  }
...
void RmArm::Arm_Force_Position_Move_Pose_Callback(
  const rm_ros_interfaces::msg::Forcepositionmovepose::SharedPtr msg)
{
  int32_t res;
  ...
  res = Rm_Api.rm_force_position_move_pose(
    robot_handle, joint_pose, sensor, mode, dir, force,
    follow);
  if (res != 0) {
...
void RmArm::Arm_Force_Position_Move_Callback(
  const rm_ros_interfaces::msg::Forcepositionmove::SharedPtr msg)
{
  int16_t res = 0;
  ...
  for (std::size_t i = 0; i < msg->joint.size(); i++) {
...
void RmArm::Arm_Set_Force_Postion_Callback(
  const rm_ros_interfaces::msg::Setforceposition::SharedPtr msg)
{
  int32_t res;
...
void RmArm::Arm_Set_Joint_Err_Clear_Callback(
  const rm_ros_interfaces::msg::Jointerrclear::SharedPtr msg)
{
  int joint_num;
  // bool block;
  int32_t res;
...
void RmArm::Arm_Set_Gripper_Pick_On_Callback(
  const rm_ros_interfaces::msg::Gripperpick::SharedPtr msg)
{
  int speed;
  int force;
  bool block;
...
void RmArm::Arm_Set_Gripper_Position_Callback(
  const rm_ros_interfaces::msg::Gripperset::SharedPtr msg)
{
  int position;
  bool block;
  int timeout;
...
void RmArm::Arm_Set_Hand_Follow_Angle_Callback(
  const rm_ros_interfaces::msg::Handangle::SharedPtr msg)
{
  int angle[6];
  int block;
  int32_t res;
...
void Udp_Robot_Status_Callback(rm_realtime_arm_joint_state_t data)
{
  ...
  if (rm_plus_base_g == true) {
    // The SDK 1.1.6 source arrays contain six device degrees of freedom,
    // while the ROS-facing compatibility message reserves twelve slots.
    // Clear every destination slot so unused entries cannot retain stale
    // data, then copy only the elements provided by the SDK.
    for (int i = 0; i < 12; i++) {
  ...
  if (rm_plus_state_g == true) {
    ...
    for (int i = 0; i < 18; i++) {
      ...
      Udp_RM_Joint.udp_rm_plus_state_info.tangential_force[i] =
        data.plus_state_info.tangential_force[i];
      Udp_RM_Joint.udp_rm_plus_state_info.tangential_force_dir[i] =
        data.plus_state_info.tangential_force_dir[i];
  ...
  if (connect_state == 0) {
    ...
    if (rm_plus_state_g == true) {
      ...
      for (int i = 0; i < 12; i++) {
        ...
        udp_rm_plus_state_.tangential_force[i] =
          Udp_RM_Joint.udp_rm_plus_state_info.tangential_force[i];
        udp_rm_plus_state_.tangential_force_dir[i] =
          Udp_RM_Joint.udp_rm_plus_state_info.tangential_force_dir[i];
    ...
    const int socket_result = Arm_Socket_Start_Connect();
    if (socket_result != 0) {
      RCLCPP_INFO(
        this->get_logger(),
        "Waiting to reconnect to %s:%d (TCP probe error %d)",
        tcp_ip, tcp_port, socket_result);
      return;
    }

    const int start_result = Arm_Start();
    if (start_result != 0) {
      RCLCPP_ERROR(
        this->get_logger(),
        "SDK reconnect to %s:%d failed with error %d",
        tcp_ip, tcp_port, start_result);
      return;
    }
    come_time = 0;
    connect_state = 0;
    connect_state_flag = 0;
    RCLCPP_INFO(this->get_logger(), "Reconnected to %s:%d", tcp_ip, tcp_port);
  }
}
...
bool UdpPublisherNode::read_data()
{
  ...
  ssize_t numBytes = recvfrom(
    16, udp_socket_buffer, sizeof(udp_socket_buffer), 0,
...
: rclcpp::Node("udp_publish_node")
{
  ...
  Arm_Position_Result = this->create_publisher<geometry_msgs::msg::Pose>(
    "rm_driver/udp_arm_position", 10);  // 发布当前的关节姿态
  Six_Force_Result = this->create_publisher<rm_ros_interfaces::msg::Sixforce>(
    "rm_driver/udp_six_force", 10);  // 发布当前的原始六维力数据
  Six_Zero_Force_Result = this->create_publisher<rm_ros_interfaces::msg::Sixforce>(
    "rm_driver/udp_six_zero_force", 10);  // 发布当前标坐标系下六维力数据
  One_Force_Result = this->create_publisher<rm_ros_interfaces::msg::Sixforce>(
    "rm_driver/udp_one_force", 10);  // 发布当前的原始一维力数据
  One_Zero_Force_Result = this->create_publisher<rm_ros_interfaces::msg::Sixforce>(
    "rm_driver/udp_one_zero_force", 10);  // 发布当前目标坐标系下一维力数据
  ...
  Arm_Coordinate_Result = this->create_publisher<std_msgs::msg::UInt16>(
    "rm_driver/udp_arm_coordinate", 10);  // 发布当前六维力数据的基准坐标系
  Hand_Status_Result = this->create_publisher<rm_ros_interfaces::msg::Handstatus>(
    "rm_driver/udp_hand_status", 10);  // 发布灵巧手状态数据

  Arm_Current_Status_Result = this->create_publisher<rm_ros_interfaces::msg::Armcurrentstatus>(
    "rm_driver/udp_arm_current_status", 10);
  Joint_Current_Result = this->create_publisher<rm_ros_interfaces::msg::Jointcurrent>(
    "rm_driver/udp_joint_current", 10);
  Joint_En_Flag_Result = this->create_publisher<rm_ros_interfaces::msg::Jointenflag>(
    "rm_driver/udp_joint_en_flag", 10);
  Joint_Pose_Euler_Result = this->create_publisher<rm_ros_interfaces::msg::Jointposeeuler>(
    "rm_driver/udp_joint_pose_euler", 10);
  Joint_Speed_Result = this->create_publisher<rm_ros_interfaces::msg::Jointspeed>(
    "rm_driver/udp_joint_speed", 10);
  Joint_Temperature_Result = this->create_publisher<rm_ros_interfaces::msg::Jointtemperature>(
    "rm_driver/udp_joint_temperature", 10);
  Joint_Voltage_Result = this->create_publisher<rm_ros_interfaces::msg::Jointvoltage>(
    "rm_driver/udp_joint_voltage", 10);
  Rm_Plus_Base_Result = this->create_publisher<rm_ros_interfaces::msg::Rmplusbase>(
    "rm_driver/udp_rm_plus_base", 10);
  Rm_Plus_State_Result = this->create_publisher<rm_ros_interfaces::msg::Rmplusstate>(
    "rm_driver/udp_rm_plus_state", 10);
  Lift_State_Result = this->create_publisher<rm_ros_interfaces::msg::Udpliftstate>(
    "rm_driver/udp_lift_state", 10);
  Expand_State_Result = this->create_publisher<rm_ros_interfaces::msg::Udpexpandstate>(
    "rm_driver/udp_expand_state", 10);
  Aloha_State_Result = this->create_publisher<rm_ros_interfaces::msg::Alohastate>(
    "rm_driver/udp_aloha_state", 10);
}
...
: rclcpp::Node("rm_driver")
{
  ...
  while (rclcpp::ok()) {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Waiting for %s:%d (TCP probe error %d)",
      tcp_ip, tcp_port, socket_result);
    wait_while_running(1s);
  }
  if (!rclcpp::ok()) {
    throw ShutdownRequested("Shutdown requested while waiting for the robot connection");
  }
  if (!wait_while_running(2s)) {
    throw ShutdownRequested("Shutdown requested during driver startup");
  }
  ...
  if (start_result != 0) {
    ...
    message << "SDK connection to " << arm_ip_ << ':' << tcp_port
            << " failed with error " << start_result;
  ...
  Set_UDP_Configuration(
    udp_cycle_, udp_port_, udp_force_coordinate_, udp_ip_, udp_hand_,
    udp_rm_plus_base_, udp_rm_plus_state_);
  ...
  Get_Realtime_Push_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    ...
    sub_opt2);
  /******************************************************设置udp配置********************************************************************/
  Set_Realtime_Push_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_realtime_push_result", rclcpp::ParametersQoS());
...
/***********************************************************************运动配置**********************************************************************/
  /****************************************MoveJ运动控制*************************************/
  MoveJ_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  MoveJ_Cmd = this->create_subscription<rm_ros_interfaces::msg::Movej>(
    ...
    sub_opt4);
  /****************************************MoveL运动控制*************************************/
  MoveL_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  MoveL_Cmd = this->create_subscription<rm_ros_interfaces::msg::Movel>(
    ...
    sub_opt4);
  MoveL_offset_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/movel_offset_result", rclcpp::ParametersQoS());
  MoveL_offset_Cmd = this->create_subscription<rm_ros_interfaces::msg::Moveloffset>(
    "rm_driver/movel_offset_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_MoveL_Offset_Callback, this, std::placeholders::_1),
    sub_opt4);
  /****************************************MoveC运动控制*************************************/
  MoveC_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  Movep_CANFD_Custom_Cmd = this->create_subscription<rm_ros_interfaces::msg::Carteposcustom>(
    ...
    sub_opt4);
  /****************************************MoveJ_P运动控制*************************************/
  MoveJ_P_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  MoveJ_P_Cmd = this->create_subscription<rm_ros_interfaces::msg::Movejp>(
    ...
    sub_opt4);
  /***********************************************轨迹急停****************************************/
  Move_Stop_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  Arm_Emergency_Stop_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/emergency_stop_result", rclcpp::ParametersQoS());
  Arm_Emergency_Stop_Cmd = this->create_subscription<rm_ros_interfaces::msg::Stop>(
    "rm_driver/emergency_stop_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Emergency_Stop_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************轨迹暂停****************************************/
  Arm_Pause_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/pause_result",
    rclcpp::ParametersQoS());
  Arm_Pause_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/pause_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Pause_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************轨迹暂停后继续轨迹运动****************************************/
  Set_Arm_Continue_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_arm_continue_result", rclcpp::ParametersQoS());
  Set_Arm_Continue_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/set_arm_continue_cmd", rclcpp::ParametersQoS(),
...
/******************************************************************************示教指令*****************************************************************/
  /*********************************************************关节示教*****************************************************************/
  Set_Joint_Teach_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_joint_teach_result", rclcpp::ParametersQoS());
  Set_Joint_Teach_Cmd = this->create_subscription<rm_ros_interfaces::msg::Jointteach>(
    ...
    sub_opt4);
  /*********************************************************位置示教*****************************************************************/
  Set_Pos_Teach_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_pos_teach_result", rclcpp::ParametersQoS());
  Set_Pos_Teach_Cmd = this->create_subscription<rm_ros_interfaces::msg::Posteach>(
    ...
    sub_opt4);
  /*********************************************************姿态示教*****************************************************************/
  Set_Ort_Teach_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_ort_teach_result", rclcpp::ParametersQoS());
  Set_Ort_Teach_Cmd = this->create_subscription<rm_ros_interfaces::msg::Ortteach>(
    ...
    sub_opt4);
  /*********************************************************示教停止*****************************************************************/
  Set_Stop_Teach_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_stop_teach_result", rclcpp::ParametersQoS());
...
/******************************************************************************四代控制器新增*****************************************************************/
  ...
  //         std::bind(&RmArm::Arm_Get_Arm_Software_Info_Callback,this,std::placeholders::_1),
  //         sub_opt2);}
  Get_Arm_Software_Version_Result = this->create_publisher<rm_ros_interfaces::msg::Armsoftversion>(
    "rm_driver/get_arm_software_version_result", rclcpp::ParametersQoS());
  Get_Arm_Software_Version_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_arm_software_version_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Get_Arm_Software_Info_Callback, this, std::placeholders::_1),
    sub_opt2);
  /************************************************************************查询软件版本***************************************************************/
  Get_Robot_Info_Result = this->create_publisher<rm_ros_interfaces::msg::RobotInfo>(
    "rm_driver/get_robot_info_result", rclcpp::ParametersQoS());
  Get_Robot_Info_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_robot_info_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Get_Robot_Info_Callback, this, std::placeholders::_1),
    sub_opt2);
  /************************************************************************查询关节软件版本***************************************************************/
  Get_Joint_Software_Version_Result = this->create_publisher<rm_ros_interfaces::msg::Jointversion>(
    "rm_driver/get_joint_software_version_result", rclcpp::ParametersQoS());
  Get_Joint_Software_Version_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_joint_software_version_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Get_Joint_Software_Version_Callback, this, std::placeholders::_1),
    sub_opt2);
  /************************************************************************查询末端接口板软件版本号***************************************************************/
  Get_Tool_Software_Version_Result =
    this->create_publisher<rm_ros_interfaces::msg::Toolsoftwareversionv4>(
    "rm_driver/get_tool_software_version_result", rclcpp::ParametersQoS());
  Get_Tool_Software_Version_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_tool_software_version_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Get_Tool_Software_Version_Callback, this, std::placeholders::_1),
    sub_opt2);
  /************************************************************************ 查询流程图运行状态***************************************************************/
  Get_Flowchart_Program_Run_State_Result =
    this->create_publisher<rm_ros_interfaces::msg::Flowchartrunstate>(
    "rm_driver/get_flowchart_program_run_state_result", rclcpp::ParametersQoS());
  Get_Flowchart_Program_Run_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_flowchart_program_run_state_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Get_Flowchart_Program_Run_State_Callback, this, std::placeholders::_1),
    sub_opt2);

  /************************************************************************轨迹列表相关***************************************************************/
  Get_Trajectory_File_List_Result = this->create_publisher<rm_ros_interfaces::msg::Trajectorylist>(
    "rm_driver/get_trajectory_file_list_result", rclcpp::ParametersQoS());
  Get_Trajectory_File_List_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Gettrajectorylist>(
    "rm_driver/get_trajectory_file_list_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Get_Trajectory_File_List_Callback, this, std::placeholders::_1),
    sub_opt2);
  Set_Run_Trajectory_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_run_trajectory_result", rclcpp::ParametersQoS());
  Set_Run_Trajectory_Cmd = this->create_subscription<std_msgs::msg::String>(
    "rm_driver/set_run_trajectory_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Set_Run_Trajectory_Callback, this, std::placeholders::_1),
    sub_opt2);
  Delete_Trajectory_File_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/delete_trajectory_file_result", rclcpp::ParametersQoS());
  Delete_Trajectory_File_Cmd = this->create_subscription<std_msgs::msg::String>(
    "rm_driver/delete_trajectory_file_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Delete_Trajectory_File_Callback, this, std::placeholders::_1),
    sub_opt2);
  Save_Trajectory_File_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/save_trajectory_file_result", rclcpp::ParametersQoS());
  Save_Trajectory_File_Cmd = this->create_subscription<std_msgs::msg::String>(
    "rm_driver/save_trajectory_file_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Save_Trajectory_File_Callback, this, std::placeholders::_1),
    sub_opt2);

  /************************************************************************Modbus相关(四代控制器)***************************************************************/
  /***********************************************************************设置控制器RS485模式(三四代共用)***********************************************************************/
  Set_Controller_RS485_Mode_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_controller_rs485_mode_result", rclcpp::ParametersQoS());
  Set_Controller_RS485_Mode_Cmd = this->create_subscription<rm_ros_interfaces::msg::RS485params>(
    "rm_driver/set_controller_rs485_mode_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Set_Controller_RS485_Mode_Callback, this, std::placeholders::_1),
    sub_opt5);
  /***********************************************************************查询控制器RS485模式(三四代共用)***********************************************************************/
  Get_Controller_RS485_Mode_v4_Result = this->create_publisher<rm_ros_interfaces::msg::RS485params>(
    "rm_driver/get_controller_rs485_mode_result", rclcpp::ParametersQoS());
  Get_Controller_RS485_Mode_v4_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_controller_rs485_mode_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Get_Controller_RS485_Mode_v4_Callback, this, std::placeholders::_1),
    sub_opt5);
  /***********************************************************************查询工具端RS485模式(三四代共用)***********************************************************************/
  Get_Tool_RS485_Mode_v4_Result = this->create_publisher<rm_ros_interfaces::msg::RS485params>(
    "rm_driver/get_tool_rs485_mode_v4_result", rclcpp::ParametersQoS());
  Get_Tool_RS485_Mode_v4_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_tool_rs485_mode_cmd", rclcpp::ParametersQoS(),
  ...
  /************************************************************************新增Modbus TCP主站***************************************************************/
  if (controller_type == 4) {
    Add_Modbus_Tcp_Master_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/add_modbus_tcp_master_result", rclcpp::ParametersQoS());
    Add_Modbus_Tcp_Master_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbustcpmasterinfo>(
      "rm_driver/add_modbus_tcp_master_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Add_Modbus_Tcp_Master_Callback, this, std::placeholders::_1),
      sub_opt5);
    /************************************************************************更新Modbus TCP主站***************************************************************/
    Update_Modbus_Tcp_Master_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/update_modbus_tcp_master_result", rclcpp::ParametersQoS());
    Update_Modbus_Tcp_Master_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbustcpmasterupdata>(
      "rm_driver/update_modbus_tcp_master_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Update_Modbus_Tcp_Master_Callback, this, std::placeholders::_1),
      sub_opt5);
    /************************************************************************删除Modbus TCP主站***************************************************************/
    Delete_Modbus_Tcp_Master_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/delete_modbus_tcp_master_result", rclcpp::ParametersQoS());
    Delete_Modbus_Tcp_Master_Cmd = this->create_subscription<rm_ros_interfaces::msg::Mastername>(
      "rm_driver/delete_modbus_tcp_master_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Delete_Modbus_Tcp_Master_Callback, this, std::placeholders::_1),
      sub_opt5);
    /************************************************************************查询指定modbus主站*****************************************************************/
    Get_Modbus_Tcp_Master_Result =
      this->create_publisher<rm_ros_interfaces::msg::Modbustcpmasterinfo>(
      "rm_driver/get_modbus_tcp_master_result", rclcpp::ParametersQoS());
    Get_Modbus_Tcp_Master_Cmd = this->create_subscription<rm_ros_interfaces::msg::Mastername>(
      "rm_driver/get_modbus_tcp_master_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Get_Modbus_Tcp_Master_Callback, this, std::placeholders::_1),
      sub_opt5);
    /***********************************************************************查询modbus主站列表***********************************************************************/
    Get_Modbus_Tcp_Master_List_Result =
      this->create_publisher<rm_ros_interfaces::msg::Modbustcpmasterlist>(
      "rm_driver/get_modbus_tcp_master_list_result", rclcpp::ParametersQoS());
    Get_Modbus_Tcp_Master_List_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Getmodbustcpmasterlist>(
      "rm_driver/get_modbus_tcp_master_list_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Get_Modbus_Tcp_Master_List_Callback, this, std::placeholders::_1),
      sub_opt5);

    /***********************************************************************设置工具端RS485模式(四代控制器支持)***********************************************************************/
    Set_Tool_RS485_Mode_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/set_tool_rs485_mode_result", rclcpp::ParametersQoS());
    Set_Tool_RS485_Mode_Cmd = this->create_subscription<rm_ros_interfaces::msg::RS485params>(
      "rm_driver/set_tool_rs485_mode_cmd", rclcpp::ParametersQoS(),
    ...
    //     std::bind(&RmArm::Set_Controller_RS485_Mode_Callback,this,std::placeholders::_1),
    //     sub_opt5);
    Close_Controller_RS485_Modbus_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/close_controller_rtu_modbus_result", rclcpp::ParametersQoS());
    Close_Controller_RS485_Modbus_Cmd = this->create_subscription<std_msgs::msg::UInt16>(
      "rm_driver/close_controller_rtu_modbus_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Close_Controller_RS485_Modbus_Callback, this, std::placeholders::_1),
      sub_opt5);
    Set_Controller_Tcp_Modbus_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/set_controller_tcp_mode_result", rclcpp::ParametersQoS());
    Set_Controller_Tcp_Modbus_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbustcpmasterinfo>(
      "rm_driver/set_controller_tcp_mode_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Set_Controller_Tcp_Mode_Callback, this, std::placeholders::_1),
      sub_opt5);
    Close_Controller_Tcp_Modbus_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/close_controller_tcp_modbus_result", rclcpp::ParametersQoS());
    Close_Controller_Tcp_Modbus_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      "rm_driver/close_controller_tcp_modbus_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Close_Controller_Tcp_Modbus_Callback, this, std::placeholders::_1),
      sub_opt5);
  }

  /***********************************************************************Modbus RTU协议读线圈***********************************************************************/
  Read_Modbus_RTU_Coils_Result = this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_rtu_coils_result", rclcpp::ParametersQoS());
  Read_Modbus_RTU_Coils_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbusrtureadparams>(
    "rm_driver/read_modbus_rtu_coils_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_RTU_Coils_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus RTU协议写线圈***********************************************************************/
  Write_Modbus_RTU_Coils_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/write_modbus_rtu_coils_result", rclcpp::ParametersQoS());
  Write_Modbus_RTU_Coils_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbusrtuwriteparams>(
    "rm_driver/write_modbus_rtu_coils_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Write_Modbus_RTU_Coils_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus RTU协议读离散量输入***********************************************************************/
  Read_Modbus_RTU_Input_Status_Result =
    this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_rtu_input_status_result", rclcpp::ParametersQoS());
  Read_Modbus_RTU_Input_Status_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbusrtureadparams>(
    "rm_driver/read_modbus_rtu_input_status_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_RTU_Input_Status_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus RTU协议读保持寄存器***********************************************************************/
  Read_Modbus_RTU_Holding_Registers_Result =
    this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_rtu_holding_registers_result", rclcpp::ParametersQoS());
  Read_Modbus_RTU_Holding_Registers_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbusrtureadparams>(
    "rm_driver/read_modbus_rtu_holding_registers_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_RTU_Holding_Registers_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus RTU协议写保持寄存器***********************************************************************/
  Write_Modbus_RTU_Registers_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/write_modbus_rtu_registers_result", rclcpp::ParametersQoS());
  Write_Modbus_RTU_Registers_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbusrtuwriteparams>(
    "rm_driver/write_modbus_rtu_registers_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Write_Modbus_RTU_Registers_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus RTU协议读输入寄存器***********************************************************************/
  Read_Modbus_RTU_Input_Registers_Result =
    this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_rtu_input_registers_result", rclcpp::ParametersQoS());
  Read_Modbus_RTU_Input_Registers_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbusrtureadparams>(
    "rm_driver/read_modbus_rtu_input_registers_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_RTU_Input_Registers_Callback, this, std::placeholders::_1),
    sub_opt2);


  /***********************************************************************Modbus TCP协议读线圈***********************************************************************/
  Read_Modbus_TCP_Coils_Result = this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_tcp_coils_result", rclcpp::ParametersQoS());
  Read_Modbus_TCP_Coils_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbustcpreadparams>(
    "rm_driver/read_modbus_tcp_coils_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_TCP_Coils_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus TCP协议写线圈***********************************************************************/
  Write_Modbus_TCP_Coils_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/write_modbus_tcp_coils_result", rclcpp::ParametersQoS());
  Write_Modbus_TCP_Coils_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbustcpwriteparams>(
    "rm_driver/write_modbus_tcp_coils_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Write_Modbus_TCP_Coils_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus TCP协议读离散量输入***********************************************************************/
  Read_Modbus_TCP_Input_Status_Result =
    this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_tcp_input_status_result", rclcpp::ParametersQoS());
  Read_Modbus_TCP_Input_Status_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbustcpreadparams>(
    "rm_driver/read_modbus_tcp_input_status_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_TCP_Input_Status_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus TCP协议读保持寄存器***********************************************************************/
  Read_Modbus_TCP_Holding_Registers_Result =
    this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_tcp_holding_registers_result", rclcpp::ParametersQoS());
  Read_Modbus_TCP_Holding_Registers_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbustcpreadparams>(
    "rm_driver/read_modbus_tcp_holding_registers_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_TCP_Holding_Registers_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus TCP协议写保持寄存器***********************************************************************/
  Write_Modbus_TCP_Registers_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/write_modbus_tcp_registers_result", rclcpp::ParametersQoS());
  Write_Modbus_TCP_Registers_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbustcpwriteparams>(
    "rm_driver/write_modbus_tcp_registers_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Write_Modbus_TCP_Registers_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************Modbus TCP协议读输入寄存器***********************************************************************/
  Read_Modbus_TCP_Input_Registers_Result =
    this->create_publisher<rm_ros_interfaces::msg::Modbusreaddata>(
    "rm_driver/read_modbus_tcp_input_registers_result", rclcpp::ParametersQoS());
  Read_Modbus_TCP_Input_Registers_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Modbustcpreadparams>(
    "rm_driver/read_modbus_tcp_input_registers_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Read_Modbus_TCP_Input_Registers_Callback, this, std::placeholders::_1),
    sub_opt2);

  /***********************************************************************文件下发***********************************************************************/
  Send_Project_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/send_project_result",
    rclcpp::ParametersQoS());
  Send_Project_Cmd = this->create_subscription<rm_ros_interfaces::msg::Sendproject>(
    "rm_driver/send_project_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Send_Project_Callback, this, std::placeholders::_1),
    sub_opt2);
  /***********************************************************************查询在线编程运行状态***********************************************************************/
  Get_Program_Run_State_Result = this->create_publisher<rm_ros_interfaces::msg::Programrunstate>(
    "rm_driver/get_program_run_state_result", rclcpp::ParametersQoS());
  Get_Program_Run_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_program_run_state_cmd", rclcpp::ParametersQoS(),
...
/**********************************************************************透传力位混合控制***********************************************************/
  ...
  Stop_Force_Position_Move_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    ...
    sub_opt2);
  /********************************************************角度透传力位混合*****************************************************************/
  Force_Position_Move_Joint_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Forcepositionmovejoint>(
  ...
  Force_Position_Move_Pose_Cmd =
    this->create_subscription<rm_ros_interfaces::msg::Forcepositionmovepose>(
    ...
    sub_opt4);
  /********************************************************位姿透传力位补偿*****************************************************************/
  Force_Position_Move_Cmd = this->create_subscription<rm_ros_interfaces::msg::Forcepositionmove>(
    "rm_driver/force_position_move_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Force_Position_Move_Callback, this, std::placeholders::_1),
    sub_opt4);
  /********************************************************设置力位混合控制*******************************************************************/
  Set_Force_Postion_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  Set_Force_Postion_Cmd = this->create_subscription<rm_ros_interfaces::msg::Setforceposition>(
    ...
    sub_opt4);
  /********************************************************结束力位混合控制*******************************************************************/
  Stop_Force_Postion_Result = this->create_publisher<std_msgs::msg::Bool>(
...
/************************************************************************坐标系指令*************************************************************/
  /**********************************************************切换工作坐标系********************************************************************/
  Change_Work_Frame_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/change_work_frame_result", rclcpp::ParametersQoS());
  ...
  /**********************************************************切换工具坐标系********************************************************************/
  Change_Tool_Frame_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/change_tool_frame_result", rclcpp::ParametersQoS());
  Change_Tool_Frame_Cmd = this->create_subscription<std_msgs::msg::String>(
    "rm_driver/change_tool_frame_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Change_Tool_Frame_Callback, this, std::placeholders::_1),
    sub_opt2);

  /**********************************************************获得工作坐标系********************************************************************/
  Get_Curr_WorkFrame_Result = this->create_publisher<std_msgs::msg::String>(
    "rm_driver/get_curr_workFrame_result", rclcpp::ParametersQoS());
  Get_Curr_WorkFrame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    ...
    sub_opt2);
  /**********************************************************获得工具坐标系********************************************************************/
  Get_Current_Tool_Frame_Result = this->create_publisher<std_msgs::msg::String>(
...
/*****************************************************************************end***************************************************************/
  ...
  /**********************************************************设置工具端电源输出********************************************************************/
  Set_Tool_Voltage_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_tool_voltage_result", rclcpp::ParametersQoS());
  ...
  /**********************************************************清除机械臂错误码********************************************************************/
  Set_Joint_Err_Clear_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_joint_err_clear_result", rclcpp::ParametersQoS());
...
/********************************************************************末端工具-手爪控制****************************************************************/
  /****************************************手爪持续力控夹取**********************************/
  Set_Gripper_Pick_On_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_gripper_pick_on_result", rclcpp::ParametersQoS());
  Set_Gripper_Pick_On_Cmd = this->create_subscription<rm_ros_interfaces::msg::Gripperpick>(
    ...
    sub_opt3);
  /********************************************手爪力控夹取**********************************/
  Set_Gripper_Pick_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_gripper_pick_result", rclcpp::ParametersQoS());
  Set_Gripper_Pick_Cmd = this->create_subscription<rm_ros_interfaces::msg::Gripperpick>(
    ...
    sub_opt3);
  /*****************************************手爪到达指定位置**********************************/
  Set_Gripper_Position_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_gripper_position_result", rclcpp::ParametersQoS());
...
/********************************************************************末端工具-五指灵巧手控制************************************************************/
  /****************************************设置灵巧手手势序号**********************************/
  Set_Hand_Posture_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_hand_posture_result", rclcpp::ParametersQoS());
  Set_Hand_Posture_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handposture>(
    ...
    sub_opt3);
  /***************************************设置灵巧手动作序列序号*********************************/
  Set_Hand_Seq_Result = this->create_publisher<std_msgs::msg::Bool>(
  ...
  Set_Hand_Seq_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handseq>(
    ...
    sub_opt3);
  /*******************************************设置灵巧手角度************************************/
  Set_Hand_Angle_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_hand_angle_result", rclcpp::ParametersQoS());
  Set_Hand_Angle_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handangle>(
    ...
    sub_opt3);
  /*******************************************设置灵巧手速度************************************/
  Set_Hand_Speed_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_hand_speed_result", rclcpp::ParametersQoS());
  Set_Hand_Speed_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handspeed>(
    ...
    sub_opt3);
  /*******************************************设置灵巧手力度************************************/
  Set_Hand_Force_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_hand_force_result", rclcpp::ParametersQoS());
  Set_Hand_Force_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handforce>(
    ...
    sub_opt3);
  /*******************************************设置灵巧手角度跟随************************************/
  Set_Hand_Follow_Angle_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_hand_follow_angle_result", rclcpp::ParametersQoS());
  Set_Hand_Follow_Angle_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handangle>(
    ...
    sub_opt3);
  /*******************************************设置灵巧手姿势跟随************************************/
  Set_Hand_Follow_Pos_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_hand_follow_pos_result", rclcpp::ParametersQoS());
...
/********************************************************************升降机构************************************************************/
  /****************************************设置升降机构速度**********************************/
  Set_Lift_Speed_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_lift_speed_result", rclcpp::ParametersQoS());
  Set_Lift_Speed_Cmd = this->create_subscription<rm_ros_interfaces::msg::Liftspeed>(
    ...
    sub_opt3);
  /****************************************设置升降机构高度**********************************/
  Set_Lift_Height_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_lift_height_result", rclcpp::ParametersQoS());
  Set_Lift_Height_Cmd = this->create_subscription<rm_ros_interfaces::msg::Liftheight>(
    ...
    sub_opt3);
  /****************************************获取升降机构状态**********************************/
  Get_Lift_State_Result = this->create_publisher<rm_ros_interfaces::msg::Liftstate>(
    "rm_driver/get_lift_state_result", rclcpp::ParametersQoS());
...
/********************************************************************拓展关节************************************************************/
  /****************************************设置拓展关节速度**********************************/
  Set_Expand_Speed_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_expand_speed_result", rclcpp::ParametersQoS());
  Set_Expand_Speed_Cmd = this->create_subscription<std_msgs::msg::Int32>(
    "rm_driver/set_expand_speed_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Set_Expand_Speed_Callback, this, std::placeholders::_1),
    sub_opt3);
  /****************************************设置拓展关节高度**********************************/
  Set_Expand_Pos_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/set_expand_pos_result", rclcpp::ParametersQoS());
  Set_Expand_Pos_Cmd = this->create_subscription<rm_ros_interfaces::msg::Expandpos>(
    "rm_driver/set_expand_pos_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Set_Expand_Pos_Callback, this, std::placeholders::_1),
    sub_opt3);
  /****************************************获取拓展关节状态**********************************/
  Get_Expand_State_Result = this->create_publisher<rm_ros_interfaces::msg::Expandstate>(
    "rm_driver/get_expand_state_result", rclcpp::ParametersQoS());
  Get_Expand_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/get_expand_state_cmd", rclcpp::ParametersQoS(),
    std::bind(&RmArm::Arm_Get_Expand_State_Callback, this, std::placeholders::_1),
    sub_opt3);
/*******************************************************************************end*****************************************************************/

  /***************************************************获取机械臂当前状态********************************************/
  Get_Current_Arm_Original_State_Result =
    this->create_publisher<rm_ros_interfaces::msg::Armoriginalstate>(
...
/*********************************************************************六维力***************************************************************/
  /*****************************************************六维力数据清零**********************************************/
  Clear_Force_Data_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/clear_force_data_result", rclcpp::ParametersQoS());
...
/*********************************************************************系统配置***************************************************************/
  /*****************************************************清除系统错误**********************************************/
  Clear_System_Err_Result = this->create_publisher<std_msgs::msg::Bool>(
    "rm_driver/clear_system_err_result", rclcpp::ParametersQoS());
  Clear_System_Err_Cmd = this->create_subscription<std_msgs::msg::Empty>(
    "rm_driver/clear_system_err_cmd", rclcpp::ParametersQoS(),
