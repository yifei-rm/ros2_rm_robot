...
#include "rm_control/rm_control.h"
...
/* 三次样条插补 */
#include "rm_control/cubicSpline.h"

std::vector<double> time_from_start_;
std::vector<double> p_joint1_;
std::vector<double> p_joint2_;
std::vector<double> p_joint3_;
std::vector<double> p_joint4_;
std::vector<double> p_joint5_;
std::vector<double> p_joint6_;
std::vector<double> p_joint7_;
std::vector<double> v_joint1_;
std::vector<double> v_joint2_;
std::vector<double> v_joint3_;
std::vector<double> v_joint4_;
std::vector<double> v_joint5_;
std::vector<double> v_joint6_;
std::vector<double> v_joint7_;
std::vector<double> a_joint1_;
std::vector<double> a_joint2_;
std::vector<double> a_joint3_;
std::vector<double> a_joint4_;
std::vector<double> a_joint5_;
std::vector<double> a_joint6_;
std::vector<double> a_joint7_;

/* 存储的结构体 p2*/
struct vel_data
{
  int vector_len;      // total num
...
/* 加载关节位置数组等信息 */
bool cubicSpline::loadData(
  double * x_data, double * y_data, int count, double bound1,
  double bound2, BoundType type)
{
...
Rm_Control::Rm_Control(std::string name)
: Node(name)
{
  ...
  joint_pos_publisher = this->create_publisher<rm_ros_interfaces::msg::Jointpos>(
    "rm_driver/movej_canfd_cmd", qos);
...
void Rm_Control::execute_move(const std::shared_ptr<GoalHandleFJT> goal_handle)
{
  ...
  // Start gubs 2021/9/16: 修复 MoveIt 在同一位姿重复规划执行导致 rm_control 异常停止的 Bug。
  if (point_num > 3) {  // 当 MoveIt 规划的路点数大于 3 时进行三次样条插值。
    /* 各个关节位置 */
    ...
    std::vector<double> p_joint7(point_num);
    ...
    // joint1
    if (spline.loadData(
        time_from_start.data(), p_joint1.data(), point_num, 0, 0,
        cubicSpline::BoundType_First_Derivative))
    {
      ...
      // joint2
      if (spline.loadData(
          time_from_start.data(), p_joint2.data(), point_num, 0, 0,
          cubicSpline::BoundType_First_Derivative))
      {
        p_joint2_.clear();
        v_joint2_.clear();
        ...
        // joint3
        if (spline.loadData(
            time_from_start.data(), p_joint3.data(), point_num, 0, 0,
            cubicSpline::BoundType_First_Derivative))
        {
          p_joint3_.clear();
          v_joint3_.clear();
          ...
          // joint4
          if (spline.loadData(
              time_from_start.data(), p_joint4.data(), point_num, 0, 0,
              cubicSpline::BoundType_First_Derivative))
          {
            p_joint4_.clear();
            v_joint4_.clear();
            ...
            // joint5
            if (spline.loadData(
                time_from_start.data(), p_joint5.data(), point_num, 0, 0,
                cubicSpline::BoundType_First_Derivative))
            {
              p_joint5_.clear();
              v_joint5_.clear();
              ...
              // joint6
              if (spline.loadData(
                  time_from_start.data(), p_joint6.data(), point_num, 0, 0,
                  cubicSpline::BoundType_First_Derivative))
              {
                p_joint6_.clear();
                v_joint6_.clear();
                ...
                if (arm_type_ == 75) {
                  if (spline.loadData(
                      time_from_start.data(), p_joint7.data(), point_num, 0, 0,
                      cubicSpline::BoundType_First_Derivative))
                  {
                    p_joint7_.clear();
                    v_joint7_.clear();
...
void Rm_Control::timer_callback()
{
  ...
  if (point_changed) {
    if (p2.vector_cnt < p2.vector_len) {
      // Joint positions are published one interpolated point per timer tick.
      if (arm_type_ == 75) {
